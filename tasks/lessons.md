...
**同一族的第三種（2026-09-04 補）：排除規則落在掃描範圍外時是死程式碼，
而反向驗證會假通過。** 守則要排除封存區，但掃描範圍如果只有
`openspec/project/`，封存區根本不在路徑上——「在封存區塞一筆違規仍然綠」
會通過，**不是因為排除生效，是因為沒掃到**。
判準：**排除規則必須落在掃描範圍之內才有意義**。
驗法多一步——把排除那一行拿掉，同一筆注入要立刻變紅；
不會紅就代表那條排除從來沒有作用。
...
### 2026-09-04 — 有些一致性該用元件保證，不該寫守則

**踩到什麼**：八個列表頁的頁首各自手寫同一段結構，`add-account-lock-management`
新加的那頁三處都偏了（多了 `p-6`、頁首用 `<div>` 而非 `<header>`、
標題 `font-bold` 而非 `font-semibold`）。**typecheck / lint / 測試全綠**，
是使用者用眼睛看出來的。第一反應是「加一條守則」。

**Why**：守則擅長的是**可精確表達**的規則——「這個字串不該出現」「這兩份清單要一致」。
版面不是那種形狀：五個明細頁本來就是另一種結構（返回鍵 + 動態標題 + 行內徽章），
「最外層必須是 `flex flex-col gap-4`」會誤報它們；
而放寬到能容納它們之後，就抓不到今天這種偏差了。
**會誤報的守則會被繞過，那比沒有更糟。**

**How to apply**：判準是**「這個規則有沒有例外，而例外是不是也對」**。
沒有例外 → 守則。有正當例外 → 抽共用元件，讓寫歪這件事**沒有地方可以發生**。
⚠️ 但元件擋不住「不用它」，所以慣例仍要寫進 spec——
**spec 是給人讀的那一半，元件是給機器執行的那一半**，兩者缺一不可。
另外：抽元件時**形狀不同就開新元件**，不要加條件分支去服務第二種形狀，
那是共用元件開始腐爛的方式。

### 2026-09-05 — 修完一層就宣告收工，會停在錯的結論

**踩到什麼**：使用者回報「勾附件編輯存不了角色」，錯誤是
`Permission code 不存在：BACKEND:ATTACHMENT:VIEW`。查出前端的
`normalizePermissionCodes` 用字串推導合成了那個不存在的碼，修好、補測試、
提案裡還寫了「**不動後端。後端擋對了，是它把錯誤的碼退回來的**」。
實機一驗——**錯誤訊息換了一個，依然存不起來**：後端有一模一樣的字串推導，
要求一個自己目錄裡沒有的碼。

**Why**：靜態分析只能看到「我找到的那一份」。同一條業務規則在前後端各實作一次
是常態，而**找到第一份之後，「找到了」的感覺會關掉繼續找的動機**。
更糟的是那份提案還替後端背書了——把沒驗證過的推論寫成了斷言。

**How to apply**：**修完一層先問「這條規則還有誰在實作」**，
用規則的關鍵字去搜另一側（本次搜 `EDIT.*VIEW` 就會同時命中前後端）。
以及：**沒有實際跑過就不要在文件裡替別的元件背書**——
「後端擋對了」這種話要嘛驗過，要嘛不要寫。
判準：使用者回報的路徑，**修完一定要親自走一次**，走到成功為止而不是走到錯誤訊息變了為止。

### 2026-09-05 — mock 不看輸入，會把測試變成「舊行為的化石」

**踩到什麼**：改完 `validatePermissions` 之後，`CreateRoleService.spec.ts` 一條既有
測試紅了。它的 mock 是 `findByCodes.mockResolvedValue([permRecord('ROLE:EDIT')])`
——**不管傳什麼都回這個**。新實作會查「目錄裡有沒有 ROLE:VIEW」，
拿到的答案是「沒有」，於是不套用蘊含規則、不拋例外。

**Why**：那個 mock 隱含宣告了「目錄裡只有 EDIT」，而**現實中 ROLE 是有 VIEW 的**。
測試因此在斷言一個不可能發生的情境。舊實作不查目錄，所以這個矛盾看不出來；
新實作一查就暴露了。**不看輸入的 mock 會把「實作目前怎麼呼叫」凍結成契約。**

**How to apply**：mock 回傳值**依輸入而變**（本次改成
`jest.fn((codes) => codes.filter(c => catalog.includes(c)).map(asRecord))`），
它就會跟著實作變化而仍然正確。
另外：實作**分兩次查同一個 repo** 會讓「第二次查什麼」變成隱含契約，
naive mock 必然對不上——能一次查完就一次查完。
