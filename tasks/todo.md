...
**`fix-viewless-module-role-save` 已完成，待 commit。**
使用者回報「勾附件編輯存不了角色」。根因是**前後端各有一份**「EDIT 蘊含 VIEW」的
字串推導，都假設每個有 EDIT 的模組也有 VIEW——而附件刻意只有 EDIT。
結果 `BACKEND:ATTACHMENT:EDIT` **永遠不可能被指派給任何角色**。
⚠️ **提案原本斷言「後端擋對了、不用動」，那是錯的**——實機驗收才發現後端有同一個
缺陷，而且它才是致命的那個。兩支原本都零測試。
...
把待辦裡「可以直接動」的三項一次清空，
補上一條守則自己的掃描漏洞，
把「sidebar 藏起來但網址進得去」這個洞補上，
修掉一個用眼睛才看得出來的版面偏差，
把那個偏差的來源（各頁手寫頁首）換成共用元件，
最後修掉一個讓附件權限永遠存不進去的前後端共同缺陷。
逐支見「已完成」的索引。
...
- **需求內部的矛盾沒有任何東西看得出來**——已經發生兩次，而且是兩種尺度：
  1. **Purpose 與 Requirements 之間**（`api-account-suspension`）：
     `openspec archive` 合併 Requirements 但不碰 Purpose，兩者會漸行漸遠。
  2. **同一條 requirement 內的兩個 bullet 之間**（`ui-role-management`，2026-09-05）：
     「提交時**統一補入**缺失的 VIEW」與「模組僅提供其中一個時**本規則不套用**」
     在模組只有 EDIT 時直接打架，而**前後端的實作都照著前者做了**
     ——結果是附件權限永遠存不進去（#46 修）。
  **這個要靠人看**：改動既有能力時，除了看新寫的對不對，
  也要回頭確認**同一份文件裡沒有另一句在講相反的事**。
  ⚠️ 兩條 bullet 分開讀都合理，`openspec validate` 檢查格式不檢查語意。
  （「Purpose 留空或含 `TBD`」已由 `openspec-spec-format.spec.ts` 擋住，
  但守則只看得出「沒寫」，看不出「寫的跟內容不符」。）

- ⚠️ **路由權限的守則抓不到明細路由。**（#43 的 design D4）
  `permission-catalog-sync.spec.ts` 會比對「路由守衛與 sidebar 對同一 path 的權限碼」，
  但 `/front-users/:userId`、`/moderation/reports/:reportId` 這類**不在 sidebar 宣告裡**，
  無法從中推導權限——它們漏掛 `RequirePermission` 時**不會有東西紅**。
  目前五條明細路由都掛了，靠人維持。
  要根治得讓路由自己成為權限的單一宣告處，那會動到 `_nav-items.ts` 的結構。
...
- ✅ **列表頁的版面慣例沒有東西在守**（#44 修偏差、`share-page-header` 根治）。
  `add-account-lock-management`（#37）加的那頁三處都偏了（多了 `p-6`、頁首用 `<div>`、
  標題 `font-bold`）——**typecheck / lint / 測試全綠，是用眼睛看出來的**。
  #44 把那頁對正，`share-page-header` 抽出 `PageHeader` 元件並遷移 9 處，
  慣例也補進了 `platform-frontend-conventions`。
  ⚠️ **原本判斷「划算的時機是下一次加頁時」，實際上提前做了**——
  使用者當場要求，而八處遷移的成本比預估低。
  兩個判斷仍然有效、之後改到這塊還會用到：
  **不要加「最外層必須是 X」的守則**（五個明細頁本來就沒那層結構，
  規則放寬到容納它們就抓不到偏差，而會誤報的守則會被繞過）；
  **明細頁不納入共用元件**（返回鍵 + 動態標題 + 行內徽章是另一種形狀，
  硬套會讓元件長出條件分支）。
  ⚠️ **元件擋不住「不用它」**——所以慣例要寫在 spec 裡，那是給人讀的那一半。
...
- **e2e 有間歇性失敗**：**已發生 11 次，每次重跑都全綠，至今無法重現。**
...
| #43 | 09-04 | `guard-routes-by-permission` | sidebar 的隱藏不是保護——補 `RequirePermission` 路由守衛；權限碼型別化 + 守則比對後端目錄；「沒權限」統一成顯示訊息（`RequireRole` 一併改），刻意用「洩漏頁面存在」換「說得出原因」 |
| #44 | 09-04 | `fix/account-locks-layout` | 帳號鎖定頁的版面對齊其餘列表頁（`p-6`、頁首容器、字重三處）；**不走 openspec**——純版面、無需求變動也無決策 |
| #45 | 09-04 | `share-page-header` | 列表頁頁首抽成 `PageHeader`，9 處遷移；**刻意不加版面守則**——有正當例外的規則會誤報、會被繞過，這種一致性該用元件保證；慣例補進 spec，因為元件擋不住「不用它」 |
| #46 | 09-05 | `fix-viewless-module-role-save` | 「EDIT 蘊含 VIEW」的字串推導在前後端各有一份，都會索取模組沒有的 VIEW 碼——附件權限因此永遠存不進去；判斷改依權限目錄。⚠️ 提案原本斷言後端不用動，**是實機驗收推翻的** |
