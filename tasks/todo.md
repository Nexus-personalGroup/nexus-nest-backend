...
- [ ] implement `add-admin-moderation`（後台查詢：稽核紀錄 + 檢舉佇列）
  - **這是「看得到被撤回內容」的那條路徑**：需要 RBAC，且會是 `chat-message-single-entry`
    守則的第一筆豁免（豁免要註明「僅限後台、需 RBAC 授權、且必須留稽核紀錄」）。
  - 檢舉的 `contentSnapshot` 也只能從這條路徑讀得到——前台完全不回傳它。
  - `ChatReportRecord.status` 目前恆為 `PENDING`，佇列的第一個查詢就是它。
    `reviewedAt` / `reviewedBy` 刻意還沒加，語意取決於「處置」怎麼設計。
