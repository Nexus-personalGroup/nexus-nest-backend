...
**`guard-routes-by-permission`（#43）已完成，待 commit。**
sidebar 的隱藏**不是保護**——手動輸入網址照樣進得去，然後頁面裡每支 API 被後端
擋成 403。補上 `RequirePermission` 路由守衛，權限碼型別化，並加兩條守則。
⚠️ **`RequireRole` 的行為一併改了**（靜默導頁 → 顯示訊息）：兩種「沒權限」表現
並存比任何一種單獨存在都糟。代價是洩漏「這個頁面存在」，但 sidebar 本來就藏著它，
會手動輸入網址的人已經知道了。
