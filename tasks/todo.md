...
**第三輪審查（2026-09-03）已完成並收尾**：報告在
`pr/2026-09-03-01-23-project-review.md`，**0 個 🔴**、綜合 9.0，
六個小項由 `fix-review-followup-cleanup`（#38）做掉。

**下一步沒有明顯的候選**——待辦裡已經沒有「可以直接動」的項目（見「路線圖」的說明）。
真的要往前推的話，順序上最有價值的是**營運快照的無界 count**：
它是唯一沒有外部阻礙的技術債，但**第一步是加觀測而不是改索引**。

## 路線圖

**目前沒有已排程的序列**——候選與各自卡在什麼，見下面的「待辦」。

**待辦裡沒有任何「可以直接動」的項目了**，這是 08-20 以來第一次。
剩下的全落在三類：卡外部條件（前台 repo 未開、`seq` 缺口的設計、NestJS 上游）、
需要產品或帳務決定（CI 擋合併、帳號鎖定要不要預設開啟、建群組同意權）、
以及刻意不做（UUID 探測、`domain/exception` 攤平）。
**唯一沒有阻礙的是「營運快照的無界 count」，但它要先加觀測再選方案**（見技術債）。

### 已走完：2026-08-20 → 09-03，38 支 PR
...
再清掉「延後功能」裡最後一項（帳號鎖定列表），最後排第三輪審查並收尾它的小項。
逐支見「已完成」的索引。
...
- **營運快照每 5 秒跑兩個無界 `COUNT(*)`**（2026-09-03 審查報告問題 1）。
  `countRooms()` 完全沒有 WHERE、`countUsers()` 的 `deletedAt` 沒有索引
  （新加的兩個 GIN 是 trgm，幫不上）。兩張表都只會變大，而週期固定在
  `DASHBOARD_STREAM_INTERVAL_SEC`（預設 5 秒）。
  **`improve-admin-orientation`（#30）擴大了它的觸發面**：在那之前只有人開著
  營運總覽頁才付費（沒訂閱者就 `stop()`），之後首頁也打同一支，
  而首頁是登入後的落點。
  ⚠️ **先加觀測，不要直接選方案。** 三條路（`users` 加 `deletedAt` 索引 /
  `chat_rooms` 加 `createdAt` 讓它 index-only scan / 整份快照快取 30 秒）
  各有代價，而**現在還不知道哪個 count 最貴**。
  `observeMessageWriteSeconds` 那套已經在了，替快照加一個直方圖比猜便宜得多——
  這是這個專案自己在 `ResolveUserContextService` 示範過的判斷。
...
- **e2e 有間歇性失敗**：**已發生 9 次，每次重跑都全綠，至今無法重現。**
  ...
  **安靜不等於修好了**——中間沒有任何針對它的改動，所以這段空窗只是還沒再抽中，
  ...
  ⚠️ **本機的 e2e 全紅不要記進這裡**：#35 與 #36 各有一次本機 409 全紅，
  但兩次都有明確原因且百分之百重現（Nest re-export、模組接線）。
