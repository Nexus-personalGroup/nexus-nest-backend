...
- **master spec 的 `openspec validate --specs --strict` 有 7 支紅**
  （`platform-observability`、`platform-token-scope`、`ui-member-profile`、
  `ui-moderation` 等，2026-09-03 發現）。錯誤是 requirement 缺 `SHALL` / `MUST`。
  **沒有任何 CI 或守則在跑這個檢查**，所以它一直紅著沒人知道——
  change 資料夾的 `validate --strict` 有跑（#39 是 exit 0），紅的只有 master specs。
  修的時候要一併想：**要不要把它加進 `verify:ci`**，否則修完還是會再漂走。
  （`#33` 修過 master spec 的 `TBD` Purpose，是同一類「合併進 specs/ 之後就沒人再驗」的問題。）
