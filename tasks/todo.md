...
**沒有在飛的 change。** 下一步的候選與各自卡在什麼見「待辦」；
唯一「可以做、也沒有東西擋著」的是**帳號鎖定管理 CRUD**
（在「延後功能」，它的延後理由「優先度低於 M2–M4」已經失效）。

另一個值得排的是**分支審查**：第二輪審查（08-30）之後累積了 #29–#36 共八支 PR，
而本檔自己記著「change 合併後排一次審查，比累積到季末一起看划算」。
這批裡已經出過 #32 → #34 的漏網。
...
再把累積的小技術債一次清掉，最後補上兩項審查報告當初判斷「先不做」的修正。
逐支見「已完成」的索引。
...
- **Docker build cache 不會自己清到夠小**：`~/.orbstack/config/docker.json` 是空的 `{}`，
  走 Docker 預設門檻——實測長到 19GB（16.5GB 可回收）才開始清。
...
- **e2e 有間歇性失敗**：**已發生 8 次，每次重跑都全綠，至今無法重現。**
  ...
  **現況**：最後一次是 08-28（第 8 次），此後 #29–#35 共七支 PR 的 CI E2E **都一次過**。
...
- **帳號鎖定管理 CRUD（`add-account-lock-management`）**：後端 `GET/POST /api/admin/security/locks`、`DELETE …/:id`；前端 `/security/account-locks` 列表頁。沿用 SUPERADMIN role gate。
  ⚠️ **原本的延後理由是「優先度低於 M2–M4」，而 M2–M4 都做完了——那個理由已經失效。**
  它現在是待辦裡唯一沒有外部相依、可以直接動的功能。
