...
- **資料保留期限**（已在四個 change 的 Open Questions 出現，升級為獨立待辦）：
  訊息、稽核紀錄、檢舉（含內容快照）目前**全部無限保留**。這已經不只是「還沒決定」——
  資料只會越積越多，而三張表都有內容或可回推的個資。要決定的：
  各自的保留期限、清理的批次方式（`PurgeLogsService` 有現成的 `ctid IN (...)` 分批模式）、
  以及**檢舉的快照要不要跟著訊息一起清**（不清的話它會變成訊息被清後唯一的內容副本）。
- **處置動作**（`add-admin-moderation` 刻意不做）：移除訊息、停用帳號。
  - **移除訊息**要新欄位區分「使用者撤回」與「管理員移除」——兩者對客戶端的語意不同
    （「已收回」vs「因違規被移除」），共用一個欄位會讓前台無法正確顯示；還要推播同步。
  - **停權**連動 `tokenVersion`（強制登出）、WS 連線斷開、「停權期間能不能看歷史」，
    每一項都要決定，實際上是獨立主題。
