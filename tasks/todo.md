...
**下一步只剩一個候選：分支審查。** 第二輪審查（08-30）之後累積了 #29–#37
共九支 PR，而本檔自己記著「change 合併後排一次審查，比累積到季末一起看划算」。
這批值得看的理由不只是量：**#32 → #34 是一個漏網**（測試全綠但畫面說假話），
而 #36 與 #37 各自暴露了一種「測試綠但沒驗到」的新形狀（見文末的教訓清單）。
同一批裡出現三次同類問題，單靠再寫一支功能不會發現第四次。

其餘候選都卡在外部條件（前台 repo 未開、`seq` 缺口的設計、NestJS 上游），
或是需要你的決定（CI 擋合併、帳號鎖定要不要預設開啟）——見「待辦」。

## 路線圖

**目前沒有已排程的序列**——候選與各自卡在什麼，見下面的「待辦」。

### 已走完：2026-08-20 → 09-03，37 支 PR
...
最後清掉「延後功能」裡最後一項（帳號鎖定列表）。逐支見「已完成」的索引。
...
- **e2e 有間歇性失敗**：**已發生 8 次，每次重跑都全綠，至今無法重現。**
  ...
  **現況**：最後一次是 08-28（第 8 次），此後 #29–#37 共九支 PR 的 CI E2E **都一次過**。
