...
- **兩套帳號體系**：後台 `members`（RBAC、密碼策略、帳號鎖定）與前台 `users`
  （顯示名稱、頭像、信箱驗證狀態）**完全獨立的兩張表**，各自的登入端點與簽發 secret。
  前台帳號可**自行註冊**（信箱驗證、重發、忘記／重設密碼），
  **未驗證的信箱能登入但聊不了天**（HTTP 與 WS 都擋）。
  聊天領域裡的每一個「member」指的都是**前台使用者**：WS 連線、三支前台 chat 端點、
  後台審閱看到的當事人、儀表板的成員數，一律指向 `users`。後台帳號不會出現在聊天室裡。
- **後端即時通訊層**：WebSocket 連線與認證、Redis presence（跨實例一致、實例死亡自動回收）、跨實例廣播、聊天室與成員關係（1:1 私聊唯一性由 DB 約束保證；加入房間需通過成員資格判斷）、訊息收發（`clientMessageId` 去重、房間內單調遞增 `seq`、斷線補齊、送訊息限流）、已讀回條與訊息撤回（軟刪除；內容保留供檢舉調查但不外流）。與訊息撤回（軟刪除；內容保留供檢舉調查但不外流）。**可觀測性**：Prometheus 自訂指標（連線數、訊息量、寫入延遲、限流觸發）與 `chat_audit_log` 行為稽核（只記無法回溯的行為，不記訊息內容）。**檢舉**：使用者可檢舉他人訊息（只檢舉訊息、同一人冪等、被檢舉者不知情；檢舉會快照當下內容供審閱）。**後台審閱**：檢舉佇列、單筆詳情（唯一能看到被撤回內容的路徑，查看會留稽核）、狀態流轉、成員行為時間軸；權限碼 `BACKEND:MODERATION:VIEW` / `EDIT` 分開。**保留策略**：稽核紀錄 180 天、檢舉判定後 365 天（未判定的永不清）；**訊息刻意不清**——清了會讓 `seq` 重新出現洞，補齊的客戶端無法區分「被清掉」與「我漏收了」。**管理員移除訊息**（與使用者撤回用不同欄位——兩者對客戶端的語意不同；可還原，移除與還原都留稽核）。**停權**（停用帳號會**主動斷開既有的 WebSocket 連線**——連線層的認證只在 handshake 執行一次，不撤銷的話被停權者只要連線還開著就能繼續送訊息；審閱側停的是**前台使用者**、帳號管理側停的是**後台管理員**，
是兩支不同的 use case——共用一支再用參數分流的話，傳錯的後果是停錯人且沒有任何錯誤訊息）。
**後台的前台會員管理**：列表／搜尋／帳號面詳情／停權／解除／強制登出，
權限碼 `BACKEND:FRONT_USER:VIEW` / `EDIT`（與帳號管理、檢舉審閱三者互不相通）。
停權有兩個入口但共用同一個 use case——分開的是授權不是行為；
**強制登出不是停權**（不動 `status`，語意是「帳號可能外洩」）。**連線層事件限流**（每條連線每秒的事件數上限，含 `ping`；與送訊息限流是兩道獨立防線，
...
檢舉、成員、聊天室三者互相連得起來。
**營運總覽** `/moderation/dashboard`：五個數字（線上人數、待處理檢舉、聊天室、成員、今日訊息）
經 SSE 每 N 秒推送；**一個實例只跑一個計時器**，沒有訂閱者時停止；
中斷時畫面明確標示數字已過期
