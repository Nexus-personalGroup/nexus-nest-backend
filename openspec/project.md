...
- **後端即時通訊層**：WebSocket 連線與認證、Redis presence（跨實例一致、實例死亡自動回收）、跨實例廣播、聊天室與成員關係（1:1 私聊唯一性由 DB 約束保證；加入房間需通過成員資格判斷）、訊息收發（`clientMessageId` 去重、房間內單調遞增 `seq`、斷線補齊、送訊息限流）、已讀回條與訊息撤回（軟刪除；內容保留供檢舉調查但不外流）。與訊息撤回（軟刪除；內容保留供檢舉調查但不外流）。**可觀測性**：Prometheus 自訂指標（連線數、訊息量、寫入延遲、限流觸發）與 `chat_audit_log` 行為稽核（只記無法回溯的行為，不記訊息內容）。**檢舉**：使用者可檢舉他人訊息（只檢舉訊息、同一人冪等、被檢舉者不知情；檢舉會快照當下內容供審閱）。**後台審閱**：檢舉佇列、單筆詳情（唯一能看到被撤回內容的路徑，查看會留稽核）、狀態流轉、成員行為時間軸；權限碼 `BACKEND:MODERATION:VIEW` / `EDIT` 分開。**保留策略**：稽核紀錄 180 天、檢舉判定後 365 天（未判定的永不清）；**訊息刻意不清**——清了會讓 `seq` 重新出現洞，補齊的客戶端無法區分「被清掉」與「我漏收了」。附件訊息、處置動作（移除訊息／停權）與訊息保留見 `tasks/todo.md`。
